...
public class PageUtil {
    ...
    public String getStrProperty(Page page, String selector, String property) {
        ...
        elementHandle(page, selector, ThreadContext.getTimeout(), ThreadContext.getInterval(),
                e -> res.set(getElementStrProperty(e, property)));
        return res.get();
    }
    ...
    public void crawSet(Page page) {
        ...
        page.evaluateOnNewDocument("() => {Object.defineProperty(window,'navigator',{value:new Proxy(navigator,{has:(target,key)=>(key==='webdriver'?false:key in target),get:(target,key)=>key==='webdriver'?false:typeof target[key]==='function'?target[key].bind(target):target[key]})})}", PageEvaluateType.FUNCTION);
...
//        page.evaluateOnNewDocument("() => {Object.defineProperty(navigator, 'plugins', {get: () => [{'description': 'Portable Document Format', 'filename': 'internal-pdf-viewer', 'length': 1, 'name': 'Chrome PDF Plugin'}, {'description': '', 'filename': 'mhjfbmdgcfjbbpaeojofohoefgiehjai', 'length': 1, 'name': 'Chromium PDF Viewer'}, {'description': '', 'filename': 'internal-nacl-plugin', 'length': 2, 'name': 'Native Client'}]})}", PageEvaluateType.FUNCTION);
        ...
        page.evaluateOnNewDocument("() => { const getParameter = WebGLRenderingContext.getParameter;WebGLRenderingContext.prototype.getParameter = function (parameter) {if (parameter === 37445) { return 'Intel Inc.'; }if (parameter === 37446) {return 'Intel(R) Iris(TM) Graphics 6100';}return getParameter(parameter);};}", PageEvaluateType.FUNCTION);
